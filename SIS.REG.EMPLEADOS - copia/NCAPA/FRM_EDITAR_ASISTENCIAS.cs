...
namespace NCAPA
{
    public partial class FRM_EDITAR_ASISTENCIAS : Frm_Base
    {
        ...
        private void ACTUALIZAR()
        {
            try
            {
                if (string.IsNullOrWhiteSpace(dtp_FechaAsis.Text) || string.IsNullOrWhiteSpace(dtp_HoraEntra.Text) || string.IsNullOrWhiteSpace(dtp_HoraSalidadAsistencia.Text) || string.IsNullOrWhiteSpace(txtEstado.Text))
                {
                    MessageBox.Show("Tienes que llenar todos los campos", "Editar Asistencia", MessageBoxButtons.OK, MessageBoxIcon.Error);
                }
                else
                {
                    MasistenciA.Id_asistencia = Convert.ToInt32(txtid.Text);
                    ...
                    int idAsistencia;
                    if (!int.TryParse(txtid.Text, out idAsistencia))
                    {
